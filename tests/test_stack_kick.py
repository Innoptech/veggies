...
# --- Tool-pin gate (issue #87 / ADR 0059): refuse to spend a session on
...
def test_expected_tool_pins_reads_the_real_containerfile():
    ...
    # independently from the same file - the binary tools from their ARGs,
    # the harness base from the overlay's FROM tag (ADR 0057).
    ...
    base = re.search(rf"^FROM {re.escape(stack_kick.TOOL_PIN_BASE_FROM)}"
                     rf":(\S+)$", text, re.M)
    assert base  # the overlay pins the base by tag
...
def test_tool_pin_keys_bound_to_containerfile_args():
    ...
    # tools: a real pinned ARG, printf'd into the manifest. The harness
    # base (ADR 0057): no ARG exists - the checkout side is the overlay's
    # FROM tag, the image side is the binary's own `opencode --version`
    # attested into the manifest at build time.
    text = _real_containerfile_text()
    args = dict(TOOL_PIN_ARG.findall(text))
    assert stack_kick.TOOL_PIN_KEYS  # a gutted tuple fails, never vacuous
    for key in stack_kick.TOOL_PIN_KEYS:
        if key == "OPENCODE_BASE":
            assert re.search(
...
def test_main_pr_never_runs_the_tool_pin_gate(monkeypatch, calls, tmp_path):
    """PR-review sessions never execute the pinned toolchain (the review
    prompt's INSPECT, NEVER EXECUTE rule: the PR tree is audited, not
    built) - the ADR 0059 gate must not block them, same exclusion as
    discussion kicks."""
    _set_pr_env(monkeypatch, tmp_path)
    monkeypatch.setattr(stack_kick, "inflight_guard", lambda *a, **k: None)

    def gated(*a, **k):
        raise AssertionError("PR kicks must not run the tool-pin gate - "
                             "review sessions never run the toolchain")
