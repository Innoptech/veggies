...
# Gated by var.enable_ovh at the root; never applied while today's garden is a
...
resource "openstack_networking_secgroup_v2" "garden" {
  name        = var.instance_name
  description = "garden: tailscale UDP + temporary bootstrap SSH"
  region      = var.region != "" ? var.region : null
}
...
resource "openstack_networking_secgroup_rule_v2" "tailscale_udp" {
  ...
  security_group_id = openstack_networking_secgroup_v2.garden.id
  region            = var.region != "" ? var.region : null
}
...
resource "openstack_compute_instance_v2" "garden" {
  name            = var.instance_name
  flavor_name     = var.flavor_name
  image_name      = var.image_name
  key_pair        = openstack_compute_keypair_v2.admin.name
  security_groups = [openstack_networking_secgroup_v2.garden.name]
  region          = var.region != "" ? var.region : null
...
resource "openstack_compute_volume_attach_v2" "data" {
  instance_id = openstack_compute_instance_v2.garden.id
