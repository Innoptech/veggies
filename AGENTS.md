...
5. **Decisions**: read `docs/adr/README.md` first. A new decision gets a new
   ...
   diff). Decision text is immutable; the `status:` frontmatter line and the
   H1 title are living metadata (ADR 0055): a PR that supersedes or amends
   an ADR updates that ADR's `status:` line in the same PR. Keep the
...
9. **veggies CLI** (`cli/veggies.py` + `cli/veggies_stack.py`, ADR
   ...
   (schema v1: model, components or capability keys, mcps, github,
   harness_containerfile (ADR 0060)). MCP servers are
   ...
   checked-out project tier carries it (ADR 0031/0049). It also refuses a
   stale stack image the same way (exit 3): the image attests its baked
   gate-tool pins at container start and the kick compares them against
   the checkout's overlay Containerfile pins (ADR 0059).    Observability
   (ADR 0034):
