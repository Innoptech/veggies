...
def cmd_up(args: argparse.Namespace) -> int:
    ...
    print("==> kube play")
    docs = render_pod(spec, infra_repo) + render_secret_docs(spec, values)
    podman("kube", "play", "-", input_text=yaml.safe_dump_all(docs, sort_keys=True))
...
def cmd_ls(args: argparse.Namespace) -> int:
    ...
    print(f"{'NAME':<20} {'STATUS':<12} {'HOST':<7} {'PORT':<6} {'MODE':<6} REPO")
    for name, s in sorted(stacks.items()):
        status = live.get(f"garden-{name}", "down")
        print(f"{name:<20} {status:<12} {s['host'] or 'local':<7} "
              f"{s['port']:<6} {s['mode']:<6} {s['repo']}")
    return 0
