...
(ADR 0059); the operator rebuilds the image and re-labels.
...
SKIP_DONE = 3  # exit code: kick skipped - already handled (ADR 0035), in-flight (ADR 0040), refused (ADR 0049), or stale-image (ADR 0059)

# Tool-pin gate (issue #87, ADR 0059): refuse to spend a session on a
...
# the overlay Containerfile's ARGs plus the harness binary's own version
# (ADR 0057's overlay pins the base by tag, so `opencode --version` at
# build time is the pinned base's version), published into the workspace
# at container start (cli/components/opencode.py), read back over
# /file/content.
...
TOOL_PIN_KEYS = ("GITLEAK", "ACTIONLINT", "TOFU", "TFLINT", "MASK",
                 "OPENCODE_BASE")
# The expected OPENCODE_BASE_VERSION is the overlay's FROM tag (no ARG -
# tests/test_veggies.py pins the overlay FROM to the literal base image).
# The literal prefix duplicates IMAGE_OPENCODE_BASE: a base rename loses
# the key here, which the binding test in tests/test_stack_kick.py turns
# into a loud CI failure.
TOOL_PIN_BASE_FROM = "localhost/veggies-opencode-base"
...
def expected_tool_pins(repo_root: Path | str | None = None) -> dict[str, str] | None:
    """The gate-tool pins the kicked session will be held to: the
    TOOL_PIN_KEYS' <K>_VERSION values in the repo's overlay Containerfile -
    ARG lines for the binary tools, the FROM tag for OPENCODE_BASE (the
    overlay pins the base by tag, ADR 0057). A None root anchors to this
    vendored script's own repo root
    ...
    base = re.search(rf"^FROM {re.escape(TOOL_PIN_BASE_FROM)}:(\S+)$",
                     text, re.M)
    if base:
        args["OPENCODE_BASE_VERSION"] = base.group(1)
...
def main() -> int:
    ...
    if rc is not None:
        return rc
    # Tool-pin gate (issue #87, ADR 0059): never spend a session on an
...
def main_pr(number: str) -> int:
    ...
    if rc is not None:
        return rc
    # No tool-pin gate (issue #87, ADR 0059): the review session never
    # runs the pinned toolchain - INSPECT, NEVER EXECUTE means the PR tree
    # is audited, not built (same exclusion as discussion kicks).
