...
`status:` frontmatter line is living metadata (ADR 0053) - a PR that
supersedes or amends an ADR updates that ADR's `status:` line in the same
PR. ADRs record *why* a decision was taken. The system as it is today lives
in [../architecture.md](../architecture.md); operational procedures in
[../runbook.md](../runbook.md). Keep both current as reality changes.

The index below is GENERATED by `scripts/adr_index.py` from the ADR files
(number from the filename, title from the H1, status from the frontmatter)
- never hand-edit the table. The pre-commit hook regenerates it and fails
on drift; stage the result (kicked sessions: the regenerated README is part
of your diff). A rebase conflict in this file is never semantic: take
either side, run `scripts/adr_index.py`, and if it exits 2 on a duplicate
number, rename your ADR to the next free number and re-run - never
hand-merge rows. The deviation ledger under the index stays
hand-maintained.
...
| [0053](0053-generated-adr-index.md) | Generate the ADR index; the frontmatter status line is living metadata | accepted |
<!-- adr-index:end -->
...
| sops + age | ansible-vault | ADR 0004 (record never landed - #92) |
| ufw | firewalld (Fedora-native) | ADR 0008 |
| fail2ban | CrowdSec + nftables bouncer + auditd | ADR 0010 |
| Docker daemon | rootless Podman + Quadlet, per-user | ADR 0009 (record never landed - #92) |
