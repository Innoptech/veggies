...
of your diff). A rebase conflict in the generated table is never
semantic: take either side, run `scripts/adr_index.py`, and if it exits 2
on a duplicate number, rename YOUR branch's ADR to the next free number
and re-run (the error names both files; one of them is already on main) -
never hand-merge rows. The deviation ledger under the index stays
hand-maintained.
