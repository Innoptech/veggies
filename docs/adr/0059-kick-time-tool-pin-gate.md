---
status: accepted (renumbered from 0053 - main landed its own 0053..0058 mid-flight)
date: 2026-09-12
---

# 0059. Kick-time tool-pin gate: the image attests its toolchain pins, stale images are refused
...
with empty content, basic-auth is enforced, path escapes are refused,
and the `path` query param is NOT percent-decoded (an encoded `%2F`
reads as a different, nonexistent file - the gate therefore sends
literal slashes). So the stack cannot be ASKED what it runs - it must
ATTEST.
...
1. **The image attests.** The overlay Containerfile
   (`deploy/images/opencode.Containerfile`, ADR
   [0057](0057-harness-base-image-and-repo-overlay.md)'s toolchain
   layer) printf's the five pinned binary ARGs - GITLEAK, ACTIONLINT,
   TOFU, TFLINT (the four 0047 gate tools) and MASK (it runs the in-pod
   gate) - plus `OPENCODE_BASE_VERSION` into `/etc/veggies/tool-pins`
   at build time. The harness version has no ARG to printf: the
   overlay's FROM is the pinned base tag and
   `tests/test_veggies.py` pins that chain (base -> upstream tag+digest,
   both image constants in lockstep), so the base's own
   `opencode --version`, captured at build time, IS the pinned base's
   version. `tests/test_tool_pins.py` binds the manifest lines to their
